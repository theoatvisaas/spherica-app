...
class CustomAuthManager {
  ...
  Future signOut() async {
    ...
    sphericaAuthUserSubject.add(
      SphericaAuthUser(loggedIn: false),
    );
    persistAuthData();
  }

  Future<SphericaAuthUser?> signIn({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
  ...
  SphericaAuthUser? _updateCurrentUser({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
    ...
    final updatedUser = SphericaAuthUser(
      loggedIn: true,
      uid: authUid,
      userData: userData,
    );
    sphericaAuthUserSubject.add(updatedUser);
    persistAuthData();
    return updatedUser;
  }
  ...
  Future initialize() async {
    ...
    try {
      ...
      userData = _prefs.getString(_kUserDataKey) != null
          ? UserMeStruct.fromSerializableMap(
